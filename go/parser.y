%{

//TODO Put your favorite license here
		
// yacc source generated by ebnf2y[1]
// at 2014-02-07 22:33:44.009474414 +0100 CET
//
//  $ ebnf2y -o parser.y -pkg parser parser.ebnf
//
// CAUTION: If this file is a Go source file (*.go), it was generated
// automatically by '$ go tool yacc' from a *.y file - DO NOT EDIT in that case!
// 
//   [1]: http://github.com/cznic/ebnf2y

package parser //TODO real package name

//TODO required only be the demo _dump function
import (
	"bytes"
	"fmt"
	"strings"

	"github.com/cznic/strutil"
)

%}

%union {
	item interface{} //TODO insert real field(s)
}

%token	ADD_ASGN
%token	ANDAND
%token	ANDNOT
%token	ANDNOT_ASGN
%token	AND_ASGN
%token	COLAS
%token	COMM
%token	DDD
%token	DEC
%token	DIV_ASGN
%token	EQ
%token	FLOAT_LIT
%token	GE
%token	IDENTIFIER
%token	IMAGINARY_LIT
%token	INC
%token	INT_LIT
%token	LBR
%token	LE
%token	LSH
%token	LSH_ASGN
%token	MOD_ASGN
%token	MUL_ASGN
%token	NEQ
%token	OROR
%token	OR_ASGN
%token	PACK_NAME
%token	RSH
%token	RSH_ASGN
%token	RUNE_LIT
%token	STRING_LIT
%token	SUB_ASGN
%token	TYPE_NAME
%token	XOR_ASGN

%type	<item> 	/*TODO real type(s), if/where applicable */
	ADD_ASGN
	ANDAND
	ANDNOT
	ANDNOT_ASGN
	AND_ASGN
	COLAS
	COMM
	DDD
	DEC
	DIV_ASGN
	EQ
	FLOAT_LIT
	GE
	IDENTIFIER
	IMAGINARY_LIT
	INC
	INT_LIT
	LBR
	LE
	LSH
	LSH_ASGN
	MOD_ASGN
	MUL_ASGN
	NEQ
	OROR
	OR_ASGN
	PACK_NAME
	RSH
	RSH_ASGN
	RUNE_LIT
	STRING_LIT
	SUB_ASGN
	TYPE_NAME
	XOR_ASGN

%token BREAK
%token CASE
%token CHAN
%token CONST
%token CONTINUE
%token DEFAULT
%token DEFER
%token ELSE
%token FALLTHROUGH
%token FOR
%token FUNC
%token GO
%token GOTO
%token IF
%token IMPORT
%token INTERFACE
%token MAP
%token PACKAGE
%token RANGE
%token RETURN
%token SELECT
%token STRUCT
%token SWITCH
%token TYPE
%token VAR

%type	<item> 	/*TODO real type(s), if/where applicable */
	AnonymousField
	ArgumentList
	ArgumentList1
	ArrayLength
	ArrayType
	Assignment
	BaseType
	BasicLit
	Block
	Block2
	BreakStmt
	BreakStmt1
	BuiltinArgs
	BuiltinArgs1
	BuiltinCall
	BuiltinCall1
	Call
	Call1
	Channel
	ChannelType
	ChannelType1
	ChannelType11
	CommCase
	CommCase1
	CommClause
	CompositeLit
	Condition
	ConstDecl
	ConstDecl1
	ConstDecl11
	ConstSpec
	ConstSpec1
	ConstSpec11
	ContinueStmt
	ContinueStmt1
	Conversion
	DclName
	Declaration
	DeferStmt
	Element
	ElementList
	ElementList1
	ElementType
	ExprCaseClause
	ExprSwitchCase
	ExprSwitchStmt
	ExprSwitchStmt1
	ExprSwitchStmt2
	ExprSwitchStmt3
	ExprSwitchStmt4
	Expression
	Expression1
	ExpressionList
	ExpressionList1
	ExpressionStmt
	Factor
	Factor1
	Factor11
	FallthroughStmt
	FieldDecl
	FieldDecl1
	FieldDecl2
	ForClause
	ForClause1
	ForClause2
	ForClause3
	ForStmt
	ForStmt1
	ForStmt11
	Function
	FunctionBody
	FunctionDecl
	FunctionDecl1
	FunctionLit
	FunctionType
	GoStmt
	GotoStmt
	IdentifierList
	IdentifierList1
	IfStmt
	IfStmt1
	IfStmt11
	IfStmt2
	IfStmt21
	ImportDecl
	ImportDecl1
	ImportDecl11
	ImportPath
	ImportSpec
	ImportSpec1
	ImportSpec11
	IncDecStmt
	IncDecStmt1
	Index
	InitStmt
	InterfaceType
	InterfaceType1
	InterfaceTypeName
	KeyType
	Label
	LabeledStmt
	Literal
	LiteralType
	LiteralValue
	LiteralValue1
	MapType
	MethodDecl
	MethodDecl1
	MethodExpr
	MethodName
	MethodSpec
	Operand
	OperandName
	PackageClause
	ParameterDecl
	ParameterList
	ParameterList1
	Parameters
	Parameters1
	PointerType
	PostStmt
	PrimaryExpr
	PrimaryFactor
	PrimaryFactor1
	PrimaryFactor11
	PrimaryTerm
	PrimaryTerm1
	PrimaryTerm11
	RangeClause
	RangeClause1
	Receiver
	ReceiverType
	RecvExpr
	RecvStmt
	Result
	ReturnStmt
	ReturnStmt1
	SelectStmt
	SelectStmt1
	Selector
	SendStmt
	ShortVarDecl
	Signature
	Signature1
	SimpleStmt
	Slice
	Slice1
	Slice11
	Slice12
	Slice2
	Slice21
	SliceType
	SourceFile
	SourceFile1
	SourceFile2
	Start
	Statement
	StatementList
	StatementList1
	StatementList2
	StatementList21
	StructType
	StructType1
	SwitchStmt
	Tag
	Term
	Term1
	TopLevelDecl
	Type
	TypeAssertion
	TypeCaseClause
	TypeDecl
	TypeDecl1
	TypeDecl11
	TypeList
	TypeList1
	TypeLit
	TypeName
	TypeSpec
	TypeSwitchCase
	TypeSwitchGuard
	TypeSwitchStmt
	TypeSwitchStmt1
	TypeSwitchStmt2
	UnaryExpr
	UnaryExpr1
	VarDecl
	VarDecl1
	VarDecl11
	VarSpec
	VarSpec1
	VarSpec11

/*TODO %left, %right, ... declarations */

%start Start

%%

AnonymousField:
	TypeName
	{
		$$ = $1 //TODO 1
	}
|	'*' TypeName
	{
		$$ = []AnonymousField{"*", $2} //TODO 2
	}

ArgumentList:
	ExpressionList ArgumentList1
	{
		$$ = []ArgumentList{$1, $2} //TODO 3
	}

ArgumentList1:
	/* EMPTY */
	{
		$$ = nil //TODO 4
	}
|	DDD
	{
		$$ = $1 //TODO 5
	}

ArrayLength:
	Expression
	{
		$$ = $1 //TODO 6
	}

ArrayType:
	'[' ArrayLength ']' ElementType
	{
		$$ = []ArrayType{"[", $2, "]", $4} //TODO 7
	}

Assignment:
	ExpressionList '=' ExpressionList
	{
		$$ = []Assignment{$1, "=", $3} //TODO 8
	}
|	ExpressionList ADD_ASGN ExpressionList
	{
		$$ = []Assignment{$1, $2, $3} //TODO 9
	}
|	ExpressionList AND_ASGN ExpressionList
	{
		$$ = []Assignment{$1, $2, $3} //TODO 10
	}
|	ExpressionList ANDNOT_ASGN ExpressionList
	{
		$$ = []Assignment{$1, $2, $3} //TODO 11
	}
|	ExpressionList DIV_ASGN ExpressionList
	{
		$$ = []Assignment{$1, $2, $3} //TODO 12
	}
|	ExpressionList LSH_ASGN ExpressionList
	{
		$$ = []Assignment{$1, $2, $3} //TODO 13
	}
|	ExpressionList MOD_ASGN ExpressionList
	{
		$$ = []Assignment{$1, $2, $3} //TODO 14
	}
|	ExpressionList MUL_ASGN ExpressionList
	{
		$$ = []Assignment{$1, $2, $3} //TODO 15
	}
|	ExpressionList OR_ASGN ExpressionList
	{
		$$ = []Assignment{$1, $2, $3} //TODO 16
	}
|	ExpressionList RSH_ASGN ExpressionList
	{
		$$ = []Assignment{$1, $2, $3} //TODO 17
	}
|	ExpressionList SUB_ASGN ExpressionList
	{
		$$ = []Assignment{$1, $2, $3} //TODO 18
	}
|	ExpressionList XOR_ASGN ExpressionList
	{
		$$ = []Assignment{$1, $2, $3} //TODO 19
	}

BaseType:
	Type
	{
		$$ = $1 //TODO 20
	}

BasicLit:
	INT_LIT
	{
		$$ = $1 //TODO 21
	}
|	FLOAT_LIT
	{
		$$ = $1 //TODO 22
	}
|	IMAGINARY_LIT
	{
		$$ = $1 //TODO 23
	}
|	RUNE_LIT
	{
		$$ = $1 //TODO 24
	}
|	STRING_LIT
	{
		$$ = $1 //TODO 25
	}

Block:
	'{' StatementList '}'
	{
		$$ = []Block{"{", $2, "}"} //TODO 26
	}

Block2:
	LBR StatementList '}'
	{
		$$ = []Block2{$1, $2, "}"} //TODO 27
	}

BreakStmt:
	BREAK BreakStmt1
	{
		$$ = []BreakStmt{"break", $2} //TODO 28
	}

BreakStmt1:
	/* EMPTY */
	{
		$$ = nil //TODO 29
	}
|	Label
	{
		$$ = $1 //TODO 30
	}

BuiltinArgs:
	Type BuiltinArgs1
	{
		$$ = []BuiltinArgs{$1, $2} //TODO 31
	}
|	ArgumentList
	{
		$$ = $1 //TODO 32
	}

BuiltinArgs1:
	/* EMPTY */
	{
		$$ = nil //TODO 33
	}
|	',' ArgumentList
	{
		$$ = []BuiltinArgs1{",", $2} //TODO 34
	}

BuiltinCall:
	IDENTIFIER '(' BuiltinCall1 ')'
	{
		$$ = []BuiltinCall{$1, "(", $3, ")"} //TODO 35
	}

BuiltinCall1:
	/* EMPTY */
	{
		$$ = nil //TODO 36
	}
|	BuiltinArgs
	{
		$$ = $1 //TODO 37
	}

Call:
	'(' Call1 ')'
	{
		$$ = []Call{"(", $2, ")"} //TODO 38
	}

Call1:
	/* EMPTY */
	{
		$$ = nil //TODO 39
	}
|	ArgumentList
	{
		$$ = $1 //TODO 40
	}

Channel:
	Expression
	{
		$$ = $1 //TODO 41
	}

ChannelType:
	ChannelType1 ElementType
	{
		$$ = []ChannelType{$1, $2} //TODO 42
	}

ChannelType1:
	CHAN ChannelType11
	{
		$$ = []ChannelType1{"chan", $2} //TODO 43
	}
|	COMM CHAN
	{
		$$ = []ChannelType1{$1, "chan"} //TODO 44
	}

ChannelType11:
	/* EMPTY */
	{
		$$ = nil //TODO 45
	}
|	COMM
	{
		$$ = $1 //TODO 46
	}

CommCase:
	CASE CommCase1
	{
		$$ = []CommCase{"case", $2} //TODO 47
	}
|	DEFAULT
	{
		$$ = "default" //TODO 48
	}

CommCase1:
	SendStmt
	{
		$$ = $1 //TODO 49
	}
|	RecvStmt
	{
		$$ = $1 //TODO 50
	}

CommClause:
	CommCase ':' StatementList
	{
		$$ = []CommClause{$1, ":", $3} //TODO 51
	}

CompositeLit:
	LiteralType LiteralValue
	{
		$$ = []CompositeLit{$1, $2} //TODO 52
	}

Condition:
	Expression
	{
		$$ = $1 //TODO 53
	}

ConstDecl:
	CONST ConstDecl1
	{
		$$ = []ConstDecl{"const", $2} //TODO 54
	}

ConstDecl1:
	ConstSpec
	{
		$$ = $1 //TODO 55
	}
|	'(' ConstDecl11 ')'
	{
		$$ = []ConstDecl1{"(", $2, ")"} //TODO 56
	}

ConstDecl11:
	/* EMPTY */
	{
		$$ = []ConstDecl11(nil) //TODO 57
	}
|	ConstDecl11 ConstSpec ';'
	{
		$$ = append($1.([]ConstDecl11), $2, ";") //TODO 58
	}

ConstSpec:
	IdentifierList ConstSpec1
	{
		$$ = []ConstSpec{$1, $2} //TODO 59
	}

ConstSpec1:
	/* EMPTY */
	{
		$$ = nil //TODO 60
	}
|	ConstSpec11 '=' ExpressionList
	{
		$$ = []ConstSpec1{$1, "=", $3} //TODO 61
	}

ConstSpec11:
	/* EMPTY */
	{
		$$ = nil //TODO 62
	}
|	Type
	{
		$$ = $1 //TODO 63
	}

ContinueStmt:
	CONTINUE ContinueStmt1
	{
		$$ = []ContinueStmt{"continue", $2} //TODO 64
	}

ContinueStmt1:
	/* EMPTY */
	{
		$$ = nil //TODO 65
	}
|	Label
	{
		$$ = $1 //TODO 66
	}

Conversion:
	Type '(' Expression ')'
	{
		$$ = []Conversion{$1, "(", $3, ")"} //TODO 67
	}

DclName:
	IDENTIFIER
	{
		$$ = $1 //TODO 68
	}
|	PACK_NAME
	{
		$$ = $1 //TODO 69
	}
|	TYPE_NAME
	{
		$$ = $1 //TODO 70
	}

Declaration:
	ConstDecl
	{
		$$ = $1 //TODO 71
	}
|	TypeDecl
	{
		$$ = $1 //TODO 72
	}
|	VarDecl
	{
		$$ = $1 //TODO 73
	}

DeferStmt:
	DEFER Expression
	{
		$$ = []DeferStmt{"defer", $2} //TODO 74
	}

Element:
	Expression
	{
		$$ = $1 //TODO 75
	}
|	Expression ':' Expression
	{
		$$ = []Element{$1, ":", $3} //TODO 76
	}
|	Expression ':' LiteralValue
	{
		$$ = []Element{$1, ":", $3} //TODO 77
	}
|	LiteralValue
	{
		$$ = $1 //TODO 78
	}

ElementList:
	Element ElementList1
	{
		$$ = []ElementList{$1, $2} //TODO 79
	}

ElementList1:
	/* EMPTY */
	{
		$$ = []ElementList1(nil) //TODO 80
	}
|	ElementList1 ',' Element
	{
		$$ = append($1.([]ElementList1), ",", $3) //TODO 81
	}

ElementType:
	Type
	{
		$$ = $1 //TODO 82
	}

ExprCaseClause:
	ExprSwitchCase ':' StatementList
	{
		$$ = []ExprCaseClause{$1, ":", $3} //TODO 83
	}

ExprSwitchCase:
	CASE ExpressionList
	{
		$$ = []ExprSwitchCase{"case", $2} //TODO 84
	}
|	DEFAULT
	{
		$$ = "default" //TODO 85
	}

ExprSwitchStmt:
	SWITCH LBR ExprSwitchStmt1 '}'
	{
		$$ = []ExprSwitchStmt{"switch", $2, $3, "}"} //TODO 86
	}
|	SWITCH Expression LBR ExprSwitchStmt2 '}'
	{
		$$ = []ExprSwitchStmt{"switch", $2, $3, $4, "}"} //TODO 87
	}
|	SWITCH SimpleStmt ';' LBR ExprSwitchStmt3 '}'
	{
		$$ = []ExprSwitchStmt{"switch", $2, ";", $4, $5, "}"} //TODO 88
	}
|	SWITCH SimpleStmt ';' Expression LBR ExprSwitchStmt4 '}'
	{
		$$ = []ExprSwitchStmt{"switch", $2, ";", $4, $5, $6, "}"} //TODO 89
	}

ExprSwitchStmt1:
	/* EMPTY */
	{
		$$ = []ExprSwitchStmt1(nil) //TODO 90
	}
|	ExprSwitchStmt1 ExprCaseClause
	{
		$$ = append($1.([]ExprSwitchStmt1), $2) //TODO 91
	}

ExprSwitchStmt2:
	/* EMPTY */
	{
		$$ = []ExprSwitchStmt2(nil) //TODO 92
	}
|	ExprSwitchStmt2 ExprCaseClause
	{
		$$ = append($1.([]ExprSwitchStmt2), $2) //TODO 93
	}

ExprSwitchStmt3:
	/* EMPTY */
	{
		$$ = []ExprSwitchStmt3(nil) //TODO 94
	}
|	ExprSwitchStmt3 ExprCaseClause
	{
		$$ = append($1.([]ExprSwitchStmt3), $2) //TODO 95
	}

ExprSwitchStmt4:
	/* EMPTY */
	{
		$$ = []ExprSwitchStmt4(nil) //TODO 96
	}
|	ExprSwitchStmt4 ExprCaseClause
	{
		$$ = append($1.([]ExprSwitchStmt4), $2) //TODO 97
	}

Expression:
	Term Expression1
	{
		$$ = []Expression{$1, $2} //TODO 98
	}

Expression1:
	/* EMPTY */
	{
		$$ = []Expression1(nil) //TODO 99
	}
|	Expression1 OROR Term
	{
		$$ = append($1.([]Expression1), $2, $3) //TODO 100
	}

ExpressionList:
	Expression ExpressionList1
	{
		$$ = []ExpressionList{$1, $2} //TODO 101
	}

ExpressionList1:
	/* EMPTY */
	{
		$$ = []ExpressionList1(nil) //TODO 102
	}
|	ExpressionList1 ',' Expression
	{
		$$ = append($1.([]ExpressionList1), ",", $3) //TODO 103
	}

ExpressionStmt:
	Expression
	{
		$$ = $1 //TODO 104
	}

Factor:
	PrimaryFactor Factor1
	{
		$$ = []Factor{$1, $2} //TODO 105
	}

Factor1:
	/* EMPTY */
	{
		$$ = []Factor1(nil) //TODO 106
	}
|	Factor1 Factor11 PrimaryFactor
	{
		$$ = append($1.([]Factor1), $2, $3) //TODO 107
	}

Factor11:
	GE
	{
		$$ = $1 //TODO 108
	}
|	'>'
	{
		$$ = ">" //TODO 109
	}
|	LE
	{
		$$ = $1 //TODO 110
	}
|	'<'
	{
		$$ = "<" //TODO 111
	}
|	NEQ
	{
		$$ = $1 //TODO 112
	}
|	EQ
	{
		$$ = $1 //TODO 113
	}

FallthroughStmt:
	FALLTHROUGH
	{
		$$ = "fallthrough" //TODO 114
	}

FieldDecl:
	FieldDecl1 FieldDecl2
	{
		$$ = []FieldDecl{$1, $2} //TODO 115
	}

FieldDecl1:
	IdentifierList Type
	{
		$$ = []FieldDecl1{$1, $2} //TODO 116
	}
|	AnonymousField
	{
		$$ = $1 //TODO 117
	}

FieldDecl2:
	/* EMPTY */
	{
		$$ = nil //TODO 118
	}
|	Tag
	{
		$$ = $1 //TODO 119
	}

ForClause:
	ForClause1 ';' ForClause2 ';' ForClause3
	{
		$$ = []ForClause{$1, ";", $3, ";", $5} //TODO 120
	}

ForClause1:
	/* EMPTY */
	{
		$$ = nil //TODO 121
	}
|	InitStmt
	{
		$$ = $1 //TODO 122
	}

ForClause2:
	/* EMPTY */
	{
		$$ = nil //TODO 123
	}
|	Condition
	{
		$$ = $1 //TODO 124
	}

ForClause3:
	/* EMPTY */
	{
		$$ = nil //TODO 125
	}
|	PostStmt
	{
		$$ = $1 //TODO 126
	}

ForStmt:
	FOR ForStmt1 Block2
	{
		$$ = []ForStmt{"for", $2, $3} //TODO 127
	}

ForStmt1:
	/* EMPTY */
	{
		$$ = nil //TODO 128
	}
|	ForStmt11
	{
		$$ = $1 //TODO 129
	}

ForStmt11:
	Condition
	{
		$$ = $1 //TODO 130
	}
|	ForClause
	{
		$$ = $1 //TODO 131
	}
|	RangeClause
	{
		$$ = $1 //TODO 132
	}

Function:
	Signature FunctionBody
	{
		$$ = []Function{$1, $2} //TODO 133
	}

FunctionBody:
	Block
	{
		$$ = $1 //TODO 134
	}

FunctionDecl:
	FUNC DclName FunctionDecl1
	{
		$$ = []FunctionDecl{"func", $2, $3} //TODO 135
	}

FunctionDecl1:
	Function
	{
		$$ = $1 //TODO 136
	}
|	Signature
	{
		$$ = $1 //TODO 137
	}

FunctionLit:
	FUNC Function
	{
		$$ = []FunctionLit{"func", $2} //TODO 138
	}

FunctionType:
	FUNC Signature
	{
		$$ = []FunctionType{"func", $2} //TODO 139
	}

GoStmt:
	GO Expression
	{
		$$ = []GoStmt{"go", $2} //TODO 140
	}

GotoStmt:
	GOTO Label
	{
		$$ = []GotoStmt{"goto", $2} //TODO 141
	}

IdentifierList:
	DclName IdentifierList1
	{
		$$ = []IdentifierList{$1, $2} //TODO 142
	}

IdentifierList1:
	/* EMPTY */
	{
		$$ = []IdentifierList1(nil) //TODO 143
	}
|	IdentifierList1 ',' DclName
	{
		$$ = append($1.([]IdentifierList1), ",", $3) //TODO 144
	}

IfStmt:
	IF Expression Block2 IfStmt1
	{
		$$ = []IfStmt{"if", $2, $3, $4} //TODO 145
	}
|	IF SimpleStmt ';' Expression Block2 IfStmt2
	{
		$$ = []IfStmt{"if", $2, ";", $4, $5, $6} //TODO 146
	}

IfStmt1:
	/* EMPTY */
	{
		$$ = nil //TODO 147
	}
|	ELSE IfStmt11
	{
		$$ = []IfStmt1{"else", $2} //TODO 148
	}

IfStmt11:
	IfStmt
	{
		$$ = $1 //TODO 149
	}
|	Block
	{
		$$ = $1 //TODO 150
	}

IfStmt2:
	/* EMPTY */
	{
		$$ = nil //TODO 151
	}
|	ELSE IfStmt21
	{
		$$ = []IfStmt2{"else", $2} //TODO 152
	}

IfStmt21:
	IfStmt
	{
		$$ = $1 //TODO 153
	}
|	Block
	{
		$$ = $1 //TODO 154
	}

ImportDecl:
	IMPORT ImportDecl1
	{
		$$ = []ImportDecl{"import", $2} //TODO 155
	}

ImportDecl1:
	ImportSpec
	{
		$$ = $1 //TODO 156
	}
|	'(' ImportDecl11 ')'
	{
		$$ = []ImportDecl1{"(", $2, ")"} //TODO 157
	}

ImportDecl11:
	/* EMPTY */
	{
		$$ = []ImportDecl11(nil) //TODO 158
	}
|	ImportDecl11 ImportSpec ';'
	{
		$$ = append($1.([]ImportDecl11), $2, ";") //TODO 159
	}

ImportPath:
	STRING_LIT
	{
		$$ = $1 //TODO 160
	}

ImportSpec:
	ImportSpec1 ImportPath
	{
		$$ = []ImportSpec{$1, $2} //TODO 161
	}

ImportSpec1:
	/* EMPTY */
	{
		$$ = nil //TODO 162
	}
|	ImportSpec11
	{
		$$ = $1 //TODO 163
	}

ImportSpec11:
	'.'
	{
		$$ = "." //TODO 164
	}
|	DclName
	{
		$$ = $1 //TODO 165
	}

IncDecStmt:
	Expression IncDecStmt1
	{
		$$ = []IncDecStmt{$1, $2} //TODO 166
	}

IncDecStmt1:
	INC
	{
		$$ = $1 //TODO 167
	}
|	DEC
	{
		$$ = $1 //TODO 168
	}

Index:
	'[' Expression ']'
	{
		$$ = []Index{"[", $2, "]"} //TODO 169
	}

InitStmt:
	SimpleStmt
	{
		$$ = $1 //TODO 170
	}

InterfaceType:
	INTERFACE '{' InterfaceType1 '}'
	{
		$$ = []InterfaceType{"interface", "{", $3, "}"} //TODO 171
	}

InterfaceType1:
	/* EMPTY */
	{
		$$ = []InterfaceType1(nil) //TODO 172
	}
|	InterfaceType1 MethodSpec ';'
	{
		$$ = append($1.([]InterfaceType1), $2, ";") //TODO 173
	}

InterfaceTypeName:
	TypeName
	{
		$$ = $1 //TODO 174
	}

KeyType:
	Type
	{
		$$ = $1 //TODO 175
	}

Label:
	DclName
	{
		$$ = $1 //TODO 176
	}

LabeledStmt:
	Label ':' Statement
	{
		$$ = []LabeledStmt{$1, ":", $3} //TODO 177
	}

Literal:
	BasicLit
	{
		$$ = $1 //TODO 178
	}
|	CompositeLit
	{
		$$ = $1 //TODO 179
	}
|	FunctionLit
	{
		$$ = $1 //TODO 180
	}

LiteralType:
	StructType
	{
		$$ = $1 //TODO 181
	}
|	ArrayType
	{
		$$ = $1 //TODO 182
	}
|	'[' DDD ']' ElementType
	{
		$$ = []LiteralType{"[", $2, "]", $4} //TODO 183
	}
|	SliceType
	{
		$$ = $1 //TODO 184
	}
|	MapType
	{
		$$ = $1 //TODO 185
	}
|	TypeName
	{
		$$ = $1 //TODO 186
	}

LiteralValue:
	'{' LiteralValue1 '}'
	{
		$$ = []LiteralValue{"{", $2, "}"} //TODO 187
	}

LiteralValue1:
	/* EMPTY */
	{
		$$ = nil //TODO 188
	}
|	ElementList
	{
		$$ = $1 //TODO 189
	}

MapType:
	MAP '[' KeyType ']' ElementType
	{
		$$ = []MapType{"map", "[", $3, "]", $5} //TODO 190
	}

MethodDecl:
	FUNC Receiver MethodName MethodDecl1
	{
		$$ = []MethodDecl{"func", $2, $3, $4} //TODO 191
	}

MethodDecl1:
	Function
	{
		$$ = $1 //TODO 192
	}
|	Signature
	{
		$$ = $1 //TODO 193
	}

MethodExpr:
	ReceiverType '.' MethodName
	{
		$$ = []MethodExpr{$1, ".", $3} //TODO 194
	}

MethodName:
	DclName
	{
		$$ = $1 //TODO 195
	}

MethodSpec:
	MethodName Signature
	{
		$$ = []MethodSpec{$1, $2} //TODO 196
	}
|	InterfaceTypeName
	{
		$$ = $1 //TODO 197
	}

Operand:
	Literal
	{
		$$ = $1 //TODO 198
	}
|	OperandName
	{
		$$ = $1 //TODO 199
	}
|	MethodExpr
	{
		$$ = $1 //TODO 200
	}
|	'(' Expression ')'
	{
		$$ = []Operand{"(", $2, ")"} //TODO 201
	}

OperandName:
	IDENTIFIER
	{
		$$ = $1 //TODO 202
	}
|	PACK_NAME '.' IDENTIFIER
	{
		$$ = []OperandName{$1, ".", $3} //TODO 203
	}

PackageClause:
	PACKAGE DclName
	{
		$$ = []PackageClause{"package", $2} //TODO 204
	}

ParameterDecl:
	Type
	{
		$$ = $1 //TODO 205
	}
|	DclName Type
	{
		$$ = []ParameterDecl{$1, $2} //TODO 206
	}
|	DclName DDD Type
	{
		$$ = []ParameterDecl{$1, $2, $3} //TODO 207
	}
|	DDD Type
	{
		$$ = []ParameterDecl{$1, $2} //TODO 208
	}

ParameterList:
	ParameterDecl ParameterList1
	{
		$$ = []ParameterList{$1, $2} //TODO 209
	}

ParameterList1:
	/* EMPTY */
	{
		$$ = []ParameterList1(nil) //TODO 210
	}
|	ParameterList1 ',' ParameterDecl
	{
		$$ = append($1.([]ParameterList1), ",", $3) //TODO 211
	}

Parameters:
	'(' Parameters1 ')'
	{
		$$ = []Parameters{"(", $2, ")"} //TODO 212
	}

Parameters1:
	/* EMPTY */
	{
		$$ = nil //TODO 213
	}
|	ParameterList
	{
		$$ = $1 //TODO 214
	}

PointerType:
	'*' BaseType
	{
		$$ = []PointerType{"*", $2} //TODO 215
	}

PostStmt:
	SimpleStmt
	{
		$$ = $1 //TODO 216
	}

PrimaryExpr:
	Operand
	{
		$$ = $1 //TODO 217
	}
|	Conversion
	{
		$$ = $1 //TODO 218
	}
|	BuiltinCall
	{
		$$ = $1 //TODO 219
	}
|	PrimaryExpr Selector
	{
		$$ = []PrimaryExpr{$1, $2} //TODO 220
	}
|	PrimaryExpr Index
	{
		$$ = []PrimaryExpr{$1, $2} //TODO 221
	}
|	PrimaryExpr Slice
	{
		$$ = []PrimaryExpr{$1, $2} //TODO 222
	}
|	PrimaryExpr TypeAssertion
	{
		$$ = []PrimaryExpr{$1, $2} //TODO 223
	}
|	PrimaryExpr Call
	{
		$$ = []PrimaryExpr{$1, $2} //TODO 224
	}

PrimaryFactor:
	PrimaryTerm PrimaryFactor1
	{
		$$ = []PrimaryFactor{$1, $2} //TODO 225
	}

PrimaryFactor1:
	/* EMPTY */
	{
		$$ = []PrimaryFactor1(nil) //TODO 226
	}
|	PrimaryFactor1 PrimaryFactor11 PrimaryTerm
	{
		$$ = append($1.([]PrimaryFactor1), $2, $3) //TODO 227
	}

PrimaryFactor11:
	'^'
	{
		$$ = "^" //TODO 228
	}
|	'|'
	{
		$$ = "|" //TODO 229
	}
|	'-'
	{
		$$ = "-" //TODO 230
	}
|	'+'
	{
		$$ = "+" //TODO 231
	}

PrimaryTerm:
	UnaryExpr PrimaryTerm1
	{
		$$ = []PrimaryTerm{$1, $2} //TODO 232
	}

PrimaryTerm1:
	/* EMPTY */
	{
		$$ = []PrimaryTerm1(nil) //TODO 233
	}
|	PrimaryTerm1 PrimaryTerm11 UnaryExpr
	{
		$$ = append($1.([]PrimaryTerm1), $2, $3) //TODO 234
	}

PrimaryTerm11:
	ANDNOT
	{
		$$ = $1 //TODO 235
	}
|	'&'
	{
		$$ = "&" //TODO 236
	}
|	LSH
	{
		$$ = $1 //TODO 237
	}
|	RSH
	{
		$$ = $1 //TODO 238
	}
|	'%'
	{
		$$ = "%" //TODO 239
	}
|	'/'
	{
		$$ = "/" //TODO 240
	}
|	'*'
	{
		$$ = "*" //TODO 241
	}

RangeClause:
	RangeClause1 RANGE Expression
	{
		$$ = []RangeClause{$1, "range", $3} //TODO 242
	}

RangeClause1:
	ExpressionList '='
	{
		$$ = []RangeClause1{$1, "="} //TODO 243
	}
|	IdentifierList COLAS
	{
		$$ = []RangeClause1{$1, $2} //TODO 244
	}

Receiver:
	'(' TYPE_NAME ')'
	{
		$$ = []Receiver{"(", $2, ")"} //TODO 245
	}
|	'(' '*' TYPE_NAME ')'
	{
		$$ = []Receiver{"(", "*", $3, ")"} //TODO 246
	}
|	'(' DclName TYPE_NAME ')'
	{
		$$ = []Receiver{"(", $2, $3, ")"} //TODO 247
	}
|	'(' DclName '*' TYPE_NAME ')'
	{
		$$ = []Receiver{"(", $2, "*", $4, ")"} //TODO 248
	}

ReceiverType:
	TypeName
	{
		$$ = $1 //TODO 249
	}
|	'(' '*' TypeName ')'
	{
		$$ = []ReceiverType{"(", "*", $3, ")"} //TODO 250
	}
|	'(' ReceiverType ')'
	{
		$$ = []ReceiverType{"(", $2, ")"} //TODO 251
	}

RecvExpr:
	Expression
	{
		$$ = $1 //TODO 252
	}

RecvStmt:
	RecvExpr
	{
		$$ = $1 //TODO 253
	}
|	IdentifierList COLAS RecvExpr
	{
		$$ = []RecvStmt{$1, $2, $3} //TODO 254
	}
|	ExpressionList '=' RecvExpr
	{
		$$ = []RecvStmt{$1, "=", $3} //TODO 255
	}

Result:
	Parameters
	{
		$$ = $1 //TODO 256
	}
|	Type
	{
		$$ = $1 //TODO 257
	}

ReturnStmt:
	RETURN ReturnStmt1
	{
		$$ = []ReturnStmt{"return", $2} //TODO 258
	}

ReturnStmt1:
	/* EMPTY */
	{
		$$ = nil //TODO 259
	}
|	ExpressionList
	{
		$$ = $1 //TODO 260
	}

SelectStmt:
	SELECT '{' SelectStmt1 '}'
	{
		$$ = []SelectStmt{"select", "{", $3, "}"} //TODO 261
	}

SelectStmt1:
	/* EMPTY */
	{
		$$ = []SelectStmt1(nil) //TODO 262
	}
|	SelectStmt1 CommClause
	{
		$$ = append($1.([]SelectStmt1), $2) //TODO 263
	}

Selector:
	'.' IDENTIFIER
	{
		$$ = []Selector{".", $2} //TODO 264
	}

SendStmt:
	Channel COMM Expression
	{
		$$ = []SendStmt{$1, $2, $3} //TODO 265
	}

ShortVarDecl:
	IdentifierList COLAS ExpressionList
	{
		$$ = []ShortVarDecl{$1, $2, $3} //TODO 266
	}

Signature:
	Parameters Signature1
	{
		$$ = []Signature{$1, $2} //TODO 267
	}

Signature1:
	/* EMPTY */
	{
		$$ = nil //TODO 268
	}
|	Result
	{
		$$ = $1 //TODO 269
	}

SimpleStmt:
	Assignment
	{
		$$ = $1 //TODO 270
	}
|	ExpressionStmt
	{
		$$ = $1 //TODO 271
	}
|	IncDecStmt
	{
		$$ = $1 //TODO 272
	}
|	SendStmt
	{
		$$ = $1 //TODO 273
	}
|	ShortVarDecl
	{
		$$ = $1 //TODO 274
	}

Slice:
	'[' Slice1
	{
		$$ = []Slice{"[", $2} //TODO 275
	}
|	Slice2 ']'
	{
		$$ = []Slice{$1, "]"} //TODO 276
	}

Slice1:
	Slice11 ':' Slice12
	{
		$$ = []Slice1{$1, ":", $3} //TODO 277
	}

Slice11:
	/* EMPTY */
	{
		$$ = nil //TODO 278
	}
|	Expression
	{
		$$ = $1 //TODO 279
	}

Slice12:
	/* EMPTY */
	{
		$$ = nil //TODO 280
	}
|	Expression
	{
		$$ = $1 //TODO 281
	}

Slice2:
	Slice21 ':' Expression ':' Expression
	{
		$$ = []Slice2{$1, ":", $3, ":", $5} //TODO 282
	}

Slice21:
	/* EMPTY */
	{
		$$ = nil //TODO 283
	}
|	Expression
	{
		$$ = $1 //TODO 284
	}

SliceType:
	'[' ']' ElementType
	{
		$$ = []SliceType{"[", "]", $3} //TODO 285
	}

SourceFile:
	PackageClause ';' SourceFile1 SourceFile2
	{
		$$ = []SourceFile{$1, ";", $3, $4} //TODO 286
	}

SourceFile1:
	/* EMPTY */
	{
		$$ = []SourceFile1(nil) //TODO 287
	}
|	SourceFile1 ImportDecl ';'
	{
		$$ = append($1.([]SourceFile1), $2, ";") //TODO 288
	}

SourceFile2:
	/* EMPTY */
	{
		$$ = []SourceFile2(nil) //TODO 289
	}
|	SourceFile2 TopLevelDecl ';'
	{
		$$ = append($1.([]SourceFile2), $2, ";") //TODO 290
	}

Start:
	SourceFile
	{
		_parserResult = $1 //TODO 291
	}

Statement:
	Declaration
	{
		$$ = $1 //TODO 292
	}
|	LabeledStmt
	{
		$$ = $1 //TODO 293
	}
|	SimpleStmt
	{
		$$ = $1 //TODO 294
	}
|	GoStmt
	{
		$$ = $1 //TODO 295
	}
|	ReturnStmt
	{
		$$ = $1 //TODO 296
	}
|	BreakStmt
	{
		$$ = $1 //TODO 297
	}
|	ContinueStmt
	{
		$$ = $1 //TODO 298
	}
|	GotoStmt
	{
		$$ = $1 //TODO 299
	}
|	FallthroughStmt
	{
		$$ = $1 //TODO 300
	}
|	Block
	{
		$$ = $1 //TODO 301
	}
|	IfStmt
	{
		$$ = $1 //TODO 302
	}
|	SwitchStmt
	{
		$$ = $1 //TODO 303
	}
|	SelectStmt
	{
		$$ = $1 //TODO 304
	}
|	ForStmt
	{
		$$ = $1 //TODO 305
	}
|	DeferStmt
	{
		$$ = $1 //TODO 306
	}

StatementList:
	StatementList1 StatementList2
	{
		$$ = []StatementList{$1, $2} //TODO 307
	}

StatementList1:
	/* EMPTY */
	{
		$$ = nil //TODO 308
	}
|	Statement
	{
		$$ = $1 //TODO 309
	}

StatementList2:
	/* EMPTY */
	{
		$$ = []StatementList2(nil) //TODO 310
	}
|	StatementList2 ';' StatementList21
	{
		$$ = append($1.([]StatementList2), ";", $3) //TODO 311
	}

StatementList21:
	/* EMPTY */
	{
		$$ = nil //TODO 312
	}
|	Statement
	{
		$$ = $1 //TODO 313
	}

StructType:
	STRUCT '{' StructType1 '}'
	{
		$$ = []StructType{"struct", "{", $3, "}"} //TODO 314
	}

StructType1:
	/* EMPTY */
	{
		$$ = []StructType1(nil) //TODO 315
	}
|	StructType1 FieldDecl ';'
	{
		$$ = append($1.([]StructType1), $2, ";") //TODO 316
	}

SwitchStmt:
	ExprSwitchStmt
	{
		$$ = $1 //TODO 317
	}
|	TypeSwitchStmt
	{
		$$ = $1 //TODO 318
	}

Tag:
	STRING_LIT
	{
		$$ = $1 //TODO 319
	}

Term:
	Factor Term1
	{
		$$ = []Term{$1, $2} //TODO 320
	}

Term1:
	/* EMPTY */
	{
		$$ = []Term1(nil) //TODO 321
	}
|	Term1 ANDAND Factor
	{
		$$ = append($1.([]Term1), $2, $3) //TODO 322
	}

TopLevelDecl:
	Declaration
	{
		$$ = $1 //TODO 323
	}
|	FunctionDecl
	{
		$$ = $1 //TODO 324
	}
|	MethodDecl
	{
		$$ = $1 //TODO 325
	}

Type:
	TypeName
	{
		$$ = $1 //TODO 326
	}
|	TypeLit
	{
		$$ = $1 //TODO 327
	}
|	'(' Type ')'
	{
		$$ = []Type{"(", $2, ")"} //TODO 328
	}

TypeAssertion:
	'.' '(' Type ')'
	{
		$$ = []TypeAssertion{".", "(", $3, ")"} //TODO 329
	}

TypeCaseClause:
	TypeSwitchCase ':' StatementList
	{
		$$ = []TypeCaseClause{$1, ":", $3} //TODO 330
	}

TypeDecl:
	TYPE TypeDecl1
	{
		$$ = []TypeDecl{"type", $2} //TODO 331
	}

TypeDecl1:
	TypeSpec
	{
		$$ = $1 //TODO 332
	}
|	'(' TypeDecl11 ')'
	{
		$$ = []TypeDecl1{"(", $2, ")"} //TODO 333
	}

TypeDecl11:
	/* EMPTY */
	{
		$$ = []TypeDecl11(nil) //TODO 334
	}
|	TypeDecl11 TypeSpec ';'
	{
		$$ = append($1.([]TypeDecl11), $2, ";") //TODO 335
	}

TypeList:
	Type TypeList1
	{
		$$ = []TypeList{$1, $2} //TODO 336
	}

TypeList1:
	/* EMPTY */
	{
		$$ = []TypeList1(nil) //TODO 337
	}
|	TypeList1 ',' Type
	{
		$$ = append($1.([]TypeList1), ",", $3) //TODO 338
	}

TypeLit:
	ArrayType
	{
		$$ = $1 //TODO 339
	}
|	StructType
	{
		$$ = $1 //TODO 340
	}
|	PointerType
	{
		$$ = $1 //TODO 341
	}
|	FunctionType
	{
		$$ = $1 //TODO 342
	}
|	InterfaceType
	{
		$$ = $1 //TODO 343
	}
|	SliceType
	{
		$$ = $1 //TODO 344
	}
|	MapType
	{
		$$ = $1 //TODO 345
	}
|	ChannelType
	{
		$$ = $1 //TODO 346
	}

TypeName:
	TYPE_NAME
	{
		$$ = $1 //TODO 347
	}
|	PACK_NAME '.' TYPE_NAME
	{
		$$ = []TypeName{$1, ".", $3} //TODO 348
	}

TypeSpec:
	DclName Type
	{
		$$ = []TypeSpec{$1, $2} //TODO 349
	}

TypeSwitchCase:
	CASE TypeList
	{
		$$ = []TypeSwitchCase{"case", $2} //TODO 350
	}
|	DEFAULT
	{
		$$ = "default" //TODO 351
	}

TypeSwitchGuard:
	PrimaryExpr '.' '(' TYPE ')'
	{
		$$ = []TypeSwitchGuard{$1, ".", "(", "type", ")"} //TODO 352
	}
|	DclName COLAS PrimaryExpr '.' '(' TYPE ')'
	{
		$$ = []TypeSwitchGuard{$1, $2, $3, ".", "(", "type", ")"} //TODO 353
	}

TypeSwitchStmt:
	SWITCH TypeSwitchGuard LBR TypeSwitchStmt1 '}'
	{
		$$ = []TypeSwitchStmt{"switch", $2, $3, $4, "}"} //TODO 354
	}
|	SWITCH SimpleStmt ';' TypeSwitchGuard LBR TypeSwitchStmt2 '}'
	{
		$$ = []TypeSwitchStmt{"switch", $2, ";", $4, $5, $6, "}"} //TODO 355
	}

TypeSwitchStmt1:
	/* EMPTY */
	{
		$$ = []TypeSwitchStmt1(nil) //TODO 356
	}
|	TypeSwitchStmt1 TypeCaseClause
	{
		$$ = append($1.([]TypeSwitchStmt1), $2) //TODO 357
	}

TypeSwitchStmt2:
	/* EMPTY */
	{
		$$ = []TypeSwitchStmt2(nil) //TODO 358
	}
|	TypeSwitchStmt2 TypeCaseClause
	{
		$$ = append($1.([]TypeSwitchStmt2), $2) //TODO 359
	}

UnaryExpr:
	PrimaryExpr
	{
		$$ = $1 //TODO 360
	}
|	UnaryExpr1 UnaryExpr
	{
		$$ = []UnaryExpr{$1, $2} //TODO 361
	}

UnaryExpr1:
	'^'
	{
		$$ = "^" //TODO 362
	}
|	'!'
	{
		$$ = "!" //TODO 363
	}
|	'-'
	{
		$$ = "-" //TODO 364
	}
|	'+'
	{
		$$ = "+" //TODO 365
	}

VarDecl:
	VAR VarDecl1
	{
		$$ = []VarDecl{"var", $2} //TODO 366
	}

VarDecl1:
	VarSpec
	{
		$$ = $1 //TODO 367
	}
|	'(' VarDecl11 ')'
	{
		$$ = []VarDecl1{"(", $2, ")"} //TODO 368
	}

VarDecl11:
	/* EMPTY */
	{
		$$ = []VarDecl11(nil) //TODO 369
	}
|	VarDecl11 VarSpec ';'
	{
		$$ = append($1.([]VarDecl11), $2, ";") //TODO 370
	}

VarSpec:
	IdentifierList VarSpec1
	{
		$$ = []VarSpec{$1, $2} //TODO 371
	}

VarSpec1:
	Type VarSpec11
	{
		$$ = []VarSpec1{$1, $2} //TODO 372
	}
|	'=' ExpressionList
	{
		$$ = []VarSpec1{"=", $2} //TODO 373
	}

VarSpec11:
	/* EMPTY */
	{
		$$ = nil //TODO 374
	}
|	'=' ExpressionList
	{
		$$ = []VarSpec11{"=", $2} //TODO 375
	}

%%

//TODO remove demo stuff below

var _parserResult interface{}

type (
	AnonymousField interface{}
	ArgumentList interface{}
	ArgumentList1 interface{}
	ArrayLength interface{}
	ArrayType interface{}
	Assignment interface{}
	BaseType interface{}
	BasicLit interface{}
	Block interface{}
	Block2 interface{}
	BreakStmt interface{}
	BreakStmt1 interface{}
	BuiltinArgs interface{}
	BuiltinArgs1 interface{}
	BuiltinCall interface{}
	BuiltinCall1 interface{}
	Call interface{}
	Call1 interface{}
	Channel interface{}
	ChannelType interface{}
	ChannelType1 interface{}
	ChannelType11 interface{}
	CommCase interface{}
	CommCase1 interface{}
	CommClause interface{}
	CompositeLit interface{}
	Condition interface{}
	ConstDecl interface{}
	ConstDecl1 interface{}
	ConstDecl11 interface{}
	ConstSpec interface{}
	ConstSpec1 interface{}
	ConstSpec11 interface{}
	ContinueStmt interface{}
	ContinueStmt1 interface{}
	Conversion interface{}
	DclName interface{}
	Declaration interface{}
	DeferStmt interface{}
	Element interface{}
	ElementList interface{}
	ElementList1 interface{}
	ElementType interface{}
	ExprCaseClause interface{}
	ExprSwitchCase interface{}
	ExprSwitchStmt interface{}
	ExprSwitchStmt1 interface{}
	ExprSwitchStmt2 interface{}
	ExprSwitchStmt3 interface{}
	ExprSwitchStmt4 interface{}
	Expression interface{}
	Expression1 interface{}
	ExpressionList interface{}
	ExpressionList1 interface{}
	ExpressionStmt interface{}
	Factor interface{}
	Factor1 interface{}
	Factor11 interface{}
	FallthroughStmt interface{}
	FieldDecl interface{}
	FieldDecl1 interface{}
	FieldDecl2 interface{}
	ForClause interface{}
	ForClause1 interface{}
	ForClause2 interface{}
	ForClause3 interface{}
	ForStmt interface{}
	ForStmt1 interface{}
	ForStmt11 interface{}
	Function interface{}
	FunctionBody interface{}
	FunctionDecl interface{}
	FunctionDecl1 interface{}
	FunctionLit interface{}
	FunctionType interface{}
	GoStmt interface{}
	GotoStmt interface{}
	IdentifierList interface{}
	IdentifierList1 interface{}
	IfStmt interface{}
	IfStmt1 interface{}
	IfStmt11 interface{}
	IfStmt2 interface{}
	IfStmt21 interface{}
	ImportDecl interface{}
	ImportDecl1 interface{}
	ImportDecl11 interface{}
	ImportPath interface{}
	ImportSpec interface{}
	ImportSpec1 interface{}
	ImportSpec11 interface{}
	IncDecStmt interface{}
	IncDecStmt1 interface{}
	Index interface{}
	InitStmt interface{}
	InterfaceType interface{}
	InterfaceType1 interface{}
	InterfaceTypeName interface{}
	KeyType interface{}
	Label interface{}
	LabeledStmt interface{}
	Literal interface{}
	LiteralType interface{}
	LiteralValue interface{}
	LiteralValue1 interface{}
	MapType interface{}
	MethodDecl interface{}
	MethodDecl1 interface{}
	MethodExpr interface{}
	MethodName interface{}
	MethodSpec interface{}
	Operand interface{}
	OperandName interface{}
	PackageClause interface{}
	ParameterDecl interface{}
	ParameterList interface{}
	ParameterList1 interface{}
	Parameters interface{}
	Parameters1 interface{}
	PointerType interface{}
	PostStmt interface{}
	PrimaryExpr interface{}
	PrimaryFactor interface{}
	PrimaryFactor1 interface{}
	PrimaryFactor11 interface{}
	PrimaryTerm interface{}
	PrimaryTerm1 interface{}
	PrimaryTerm11 interface{}
	RangeClause interface{}
	RangeClause1 interface{}
	Receiver interface{}
	ReceiverType interface{}
	RecvExpr interface{}
	RecvStmt interface{}
	Result interface{}
	ReturnStmt interface{}
	ReturnStmt1 interface{}
	SelectStmt interface{}
	SelectStmt1 interface{}
	Selector interface{}
	SendStmt interface{}
	ShortVarDecl interface{}
	Signature interface{}
	Signature1 interface{}
	SimpleStmt interface{}
	Slice interface{}
	Slice1 interface{}
	Slice11 interface{}
	Slice12 interface{}
	Slice2 interface{}
	Slice21 interface{}
	SliceType interface{}
	SourceFile interface{}
	SourceFile1 interface{}
	SourceFile2 interface{}
	Start interface{}
	Statement interface{}
	StatementList interface{}
	StatementList1 interface{}
	StatementList2 interface{}
	StatementList21 interface{}
	StructType interface{}
	StructType1 interface{}
	SwitchStmt interface{}
	Tag interface{}
	Term interface{}
	Term1 interface{}
	TopLevelDecl interface{}
	Type interface{}
	TypeAssertion interface{}
	TypeCaseClause interface{}
	TypeDecl interface{}
	TypeDecl1 interface{}
	TypeDecl11 interface{}
	TypeList interface{}
	TypeList1 interface{}
	TypeLit interface{}
	TypeName interface{}
	TypeSpec interface{}
	TypeSwitchCase interface{}
	TypeSwitchGuard interface{}
	TypeSwitchStmt interface{}
	TypeSwitchStmt1 interface{}
	TypeSwitchStmt2 interface{}
	UnaryExpr interface{}
	UnaryExpr1 interface{}
	VarDecl interface{}
	VarDecl1 interface{}
	VarDecl11 interface{}
	VarSpec interface{}
	VarSpec1 interface{}
	VarSpec11 interface{}
)
	
func _dump() {
	s := fmt.Sprintf("%#v", _parserResult)
	s = strings.Replace(s, "%", "%%", -1)
	s = strings.Replace(s, "{", "{%i\n", -1)
	s = strings.Replace(s, "}", "%u\n}", -1)
	s = strings.Replace(s, ", ", ",\n", -1)
	var buf bytes.Buffer
	strutil.IndentFormatter(&buf, ". ").Format(s)
	buf.WriteString("\n")
	a := strings.Split(buf.String(), "\n")
	for _, v := range a {
		if strings.HasSuffix(v, "(nil)") || strings.HasSuffix(v, "(nil),") {
			continue
		}
	
		fmt.Println(v)
	}
}

// End of demo stuff
