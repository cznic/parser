%{

//TODO Put your favorite license here
		
// yacc source generated by ebnf2y[1]
// at 2013-08-01 18:01:18.865153493 +0200 CEST.
//
// CAUTION: If this file is a Go source file (*.go), it was generated
// automatically by '$ go tool yacc' from a *.y file - DO NOT EDIT in that case!
// 
//   [1]: http://github.com/cznic/ebnf2y

package main //TODO real package name

//TODO required only be the demo _dump function
import (
	"bytes"
	"fmt"
	"strings"

	"github.com/cznic/strutil"
)

%}

%union {
	item interface{} //TODO insert real field(s)
}

%token	ANDAND
%token	ANDNOT
%token	ASSIGN_OP
%token	BUILT_IN_NAME
%token	CHANCOMM
%token	COLAS
%token	COMM
%token	COMMCHAN
%token	DDD
%token	DEC
%token	EQ
%token	FLOAT_LIT
%token	GE
%token	IMAGINARY_LIT
%token	INC
%token	INT_LIT
%token	LE
%token	LSH
%token	NAME
%token	NE
%token	OROR
%token	PACK_NAME
%token	RSH
%token	RUNE_LIT
%token	STRING_LIT
%token	TYPE_NAME

%type	<item> 	/*TODO real type(s), if/where applicable */
	ANDAND
	ANDNOT
	ASSIGN_OP
	BUILT_IN_NAME
	CHANCOMM
	COLAS
	COMM
	COMMCHAN
	DDD
	DEC
	EQ
	FLOAT_LIT
	GE
	IMAGINARY_LIT
	INC
	INT_LIT
	LE
	LSH
	NAME
	NE
	OROR
	PACK_NAME
	RSH
	RUNE_LIT
	STRING_LIT
	TYPE_NAME

%token BREAK
%token CASE
%token CHAN
%token CONST
%token CONTINUE
%token DEFAULT
%token DEFER
%token ELSE
%token FALLTHROUGH
%token FOR
%token FUNC
%token GO
%token GOTO
%token IF
%token IMPORT
%token INTERFACE
%token MAP
%token PACKAGE
%token RANGE
%token RETURN
%token SELECT
%token STRUCT
%token SWITCH
%token TYPE
%token VAR

%type	<item> 	/*TODO real type(s), if/where applicable */
	AnonymousField
	ArgumentList
	ArgumentList1
	ArrayLength
	ArrayType
	Assignment
	BaseType
	BasicLit
	Binary_op
	Block
	BreakStmt
	BreakStmt1
	BuiltinArgs
	BuiltinArgs1
	BuiltinCall
	BuiltinCall1
	BuiltinCall11
	Call
	Call1
	Call11
	Channel
	ChannelType
	ChannelType1
	CommCase
	CommCase1
	CommClause
	CompositeLit
	Condition
	ConstDecl
	ConstDecl1
	ConstDecl11
	ConstSpec
	ConstSpec1
	ConstSpec11
	ContinueStmt
	ContinueStmt1
	Conversion
	Conversion1
	Dclname
	Declaration
	DeferStmt
	Element
	ElementIndex
	ElementList1
	ElementType
	ExprCaseClause
	ExprSwitchCase
	ExprSwitchStmt
	ExprSwitchStmt2
	ExprSwitchStmt3
	Expression
	ExpressionList
	ExpressionList1
	ExpressionStmt
	FallthroughStmt
	FieldDecl
	FieldDecl1
	FieldDecl2
	FieldName
	ForClause
	ForClause1
	ForClause2
	ForClause3
	ForStmt
	ForStmt1
	ForStmt11
	Function
	FunctionBody
	FunctionDecl
	FunctionDecl1
	FunctionLit
	FunctionName
	FunctionType
	GoStmt
	GotoStmt
	IdentifierList
	IdentifierList1
	IfStmt
	IfStmt2
	IfStmt21
	ImportDecl
	ImportDecl1
	ImportDecl11
	ImportPath
	ImportSpec
	ImportSpec1
	ImportSpec11
	IncDecStmt
	IncDecStmt1
	Index
	InitStmt
	InterfaceType
	InterfaceType1
	InterfaceTypeName
	Key
	KeyType
	Label
	Literal
	LiteralType
	LiteralValue
	LiteralValue1
	LiteralValue11
	MapType
	MethodDecl
	MethodDecl1
	MethodExpr
	MethodName
	MethodSpec
	Operand
	OperandName
	PackageClause
	ParameterDecl
	ParameterDecl1
	ParameterDecl2
	ParameterList1
	Parameters
	Parameters1
	Parameters11
	PointerType
	PostStmt
	PrimaryExpr
	RangeClause
	RangeClause1
	Receiver
	ReceiverType
	RecvExpr
	RecvStmt
	RecvStmt11
	ReturnStmt
	ReturnStmt1
	SelectStmt
	SelectStmt1
	Selector
	SendStmt
	ShortVarDecl
	Signature
	Signature1
	Signature11
	SimpleStmt
	Slice
	Slice1
	Slice2
	SliceType
	SourceFile
	SourceFile1
	SourceFile2
	Start
	Statement
	StatementList
	StatementList1
	StatementList11
	StructType
	StructType1
	SwitchStmt
	Tag
	TopLevelDecl
	Type
	TypeAssertion
	TypeCaseClause
	TypeDecl
	TypeDecl1
	TypeDecl11
	TypeList
	TypeList1
	TypeLit
	TypeSpec
	TypeSwitchCase
	TypeSwitchGuard
	TypeSwitchStmt
	TypeSwitchStmt2
	UnaryExpr
	Value
	VarDecl
	VarDecl1
	VarDecl11
	VarSpec
	VarSpec1
	VarSpec11

/*TODO %left, %right, ... declarations */

%start Start

%%

AnonymousField:
	TYPE_NAME
	{
		$$ = $1 //TODO 1
	}
|	'*' TYPE_NAME
	{
		$$ = []AnonymousField{"*", $2} //TODO 2
	}
|	PACK_NAME '.' TYPE_NAME
	{
		$$ = []AnonymousField{$1, ".", $3} //TODO 3
	}
|	'*' PACK_NAME '.' TYPE_NAME
	{
		$$ = []AnonymousField{"*", $2, ".", $4} //TODO 4
	}

ArgumentList:
	ExpressionList ArgumentList1
	{
		$$ = []ArgumentList{$1, $2} //TODO 5
	}

ArgumentList1:
	/* EMPTY */
	{
		$$ = nil //TODO 6
	}
|	DDD
	{
		$$ = $1 //TODO 7
	}

ArrayLength:
	Expression
	{
		$$ = $1 //TODO 8
	}

ArrayType:
	'[' ArrayLength ']' ElementType
	{
		$$ = []ArrayType{"[", $2, "]", $4} //TODO 9
	}

Assignment:
	ExpressionList ASSIGN_OP ExpressionList
	{
		$$ = []Assignment{$1, $2, $3} //TODO 10
	}

BaseType:
	Type
	{
		$$ = $1 //TODO 11
	}

BasicLit:
	INT_LIT
	{
		$$ = $1 //TODO 12
	}
|	FLOAT_LIT
	{
		$$ = $1 //TODO 13
	}
|	IMAGINARY_LIT
	{
		$$ = $1 //TODO 14
	}
|	RUNE_LIT
	{
		$$ = $1 //TODO 15
	}
|	STRING_LIT
	{
		$$ = $1 //TODO 16
	}

Binary_op:
	OROR
	{
		$$ = $1 //TODO 17
	}
|	ANDAND
	{
		$$ = $1 //TODO 18
	}
|	EQ
	{
		$$ = $1 //TODO 19
	}
|	NE
	{
		$$ = $1 //TODO 20
	}
|	'<'
	{
		$$ = "<" //TODO 21
	}
|	LE
	{
		$$ = $1 //TODO 22
	}
|	'>'
	{
		$$ = ">" //TODO 23
	}
|	GE
	{
		$$ = $1 //TODO 24
	}
|	'+'
	{
		$$ = "+" //TODO 25
	}
|	'-'
	{
		$$ = "-" //TODO 26
	}
|	'|'
	{
		$$ = "|" //TODO 27
	}
|	'^'
	{
		$$ = "^" //TODO 28
	}
|	'*'
	{
		$$ = "*" //TODO 29
	}
|	'/'
	{
		$$ = "/" //TODO 30
	}
|	'%'
	{
		$$ = "%" //TODO 31
	}
|	LSH
	{
		$$ = $1 //TODO 32
	}
|	RSH
	{
		$$ = $1 //TODO 33
	}
|	'&'
	{
		$$ = "&" //TODO 34
	}
|	ANDNOT
	{
		$$ = $1 //TODO 35
	}

Block:
	'{' StatementList '}'
	{
		$$ = []Block{"{", $2, "}"} //TODO 36
	}

BreakStmt:
	BREAK BreakStmt1
	{
		$$ = []BreakStmt{"break", $2} //TODO 37
	}

BreakStmt1:
	/* EMPTY */
	{
		$$ = nil //TODO 38
	}
|	Label
	{
		$$ = $1 //TODO 39
	}

BuiltinArgs:
	Type BuiltinArgs1
	{
		$$ = []BuiltinArgs{$1, $2} //TODO 40
	}
|	ArgumentList
	{
		$$ = $1 //TODO 41
	}

BuiltinArgs1:
	/* EMPTY */
	{
		$$ = nil //TODO 42
	}
|	',' ArgumentList
	{
		$$ = []BuiltinArgs1{",", $2} //TODO 43
	}

BuiltinCall:
	BUILT_IN_NAME '(' BuiltinCall1 ')'
	{
		$$ = []BuiltinCall{$1, "(", $3, ")"} //TODO 44
	}

BuiltinCall1:
	/* EMPTY */
	{
		$$ = nil //TODO 45
	}
|	BuiltinArgs BuiltinCall11
	{
		$$ = []BuiltinCall1{$1, $2} //TODO 46
	}

BuiltinCall11:
	/* EMPTY */
	{
		$$ = nil //TODO 47
	}
|	','
	{
		$$ = "," //TODO 48
	}

Call:
	'(' Call1 ')'
	{
		$$ = []Call{"(", $2, ")"} //TODO 49
	}

Call1:
	/* EMPTY */
	{
		$$ = nil //TODO 50
	}
|	ArgumentList Call11
	{
		$$ = []Call1{$1, $2} //TODO 51
	}

Call11:
	/* EMPTY */
	{
		$$ = nil //TODO 52
	}
|	','
	{
		$$ = "," //TODO 53
	}

Channel:
	Expression
	{
		$$ = $1 //TODO 54
	}

ChannelType:
	ChannelType1 ElementType
	{
		$$ = []ChannelType{$1, $2} //TODO 55
	}

ChannelType1:
	CHAN
	{
		$$ = "chan" //TODO 56
	}
|	CHANCOMM
	{
		$$ = $1 //TODO 57
	}
|	COMMCHAN
	{
		$$ = $1 //TODO 58
	}

CommCase:
	CASE CommCase1
	{
		$$ = []CommCase{"case", $2} //TODO 59
	}
|	DEFAULT
	{
		$$ = "default" //TODO 60
	}

CommCase1:
	SendStmt
	{
		$$ = $1 //TODO 61
	}
|	RecvStmt
	{
		$$ = $1 //TODO 62
	}

CommClause:
	CommCase ':' StatementList
	{
		$$ = []CommClause{$1, ":", $3} //TODO 63
	}

CompositeLit:
	LiteralType LiteralValue
	{
		$$ = []CompositeLit{$1, $2} //TODO 64
	}

Condition:
	Expression
	{
		$$ = $1 //TODO 65
	}

ConstDecl:
	CONST ConstDecl1
	{
		$$ = []ConstDecl{"const", $2} //TODO 66
	}

ConstDecl1:
	ConstSpec
	{
		$$ = $1 //TODO 67
	}
|	'(' ConstDecl11 ')'
	{
		$$ = []ConstDecl1{"(", $2, ")"} //TODO 68
	}

ConstDecl11:
	/* EMPTY */
	{
		$$ = []ConstDecl11(nil) //TODO 69
	}
|	ConstDecl11 ConstSpec ';'
	{
		$$ = append($1.([]ConstDecl11), $2, ";") //TODO 70
	}

ConstSpec:
	IdentifierList ConstSpec1
	{
		$$ = []ConstSpec{$1, $2} //TODO 71
	}

ConstSpec1:
	/* EMPTY */
	{
		$$ = nil //TODO 72
	}
|	ConstSpec11 '=' ExpressionList
	{
		$$ = []ConstSpec1{$1, "=", $3} //TODO 73
	}

ConstSpec11:
	/* EMPTY */
	{
		$$ = nil //TODO 74
	}
|	Type
	{
		$$ = $1 //TODO 75
	}

ContinueStmt:
	CONTINUE ContinueStmt1
	{
		$$ = []ContinueStmt{"continue", $2} //TODO 76
	}

ContinueStmt1:
	/* EMPTY */
	{
		$$ = nil //TODO 77
	}
|	Label
	{
		$$ = $1 //TODO 78
	}

Conversion:
	Type '(' Expression Conversion1 ')'
	{
		$$ = []Conversion{$1, "(", $3, $4, ")"} //TODO 79
	}

Conversion1:
	/* EMPTY */
	{
		$$ = nil //TODO 80
	}
|	','
	{
		$$ = "," //TODO 81
	}

Dclname:
	BUILT_IN_NAME
	{
		$$ = $1 //TODO 82
	}
|	PACK_NAME
	{
		$$ = $1 //TODO 83
	}
|	TYPE_NAME
	{
		$$ = $1 //TODO 84
	}
|	NAME
	{
		$$ = $1 //TODO 85
	}

Declaration:
	ConstDecl
	{
		$$ = $1 //TODO 86
	}
|	TypeDecl
	{
		$$ = $1 //TODO 87
	}
|	VarDecl
	{
		$$ = $1 //TODO 88
	}

DeferStmt:
	DEFER Expression
	{
		$$ = []DeferStmt{"defer", $2} //TODO 89
	}

Element:
	Value
	{
		$$ = $1 //TODO 90
	}
|	Key ':' Value
	{
		$$ = []Element{$1, ":", $3} //TODO 91
	}

ElementIndex:
	Expression
	{
		$$ = $1 //TODO 92
	}

ElementList1:
	/* EMPTY */
	{
		$$ = []ElementList1(nil) //TODO 93
	}
|	ElementList1 ',' Element
	{
		$$ = append($1.([]ElementList1), ",", $3) //TODO 94
	}

ElementType:
	Type
	{
		$$ = $1 //TODO 95
	}

ExprCaseClause:
	ExprSwitchCase ':' StatementList
	{
		$$ = []ExprCaseClause{$1, ":", $3} //TODO 96
	}

ExprSwitchCase:
	CASE ExpressionList
	{
		$$ = []ExprSwitchCase{"case", $2} //TODO 97
	}
|	DEFAULT
	{
		$$ = "default" //TODO 98
	}

ExprSwitchStmt:
	SWITCH ExprSwitchStmt2 '{' ExprSwitchStmt3 '}'
	{
		$$ = []ExprSwitchStmt{"switch", $2, "{", $4, "}"} //TODO 99
	}
|	SWITCH SimpleStmt ';' ExprSwitchStmt2 '{' ExprSwitchStmt3 '}'
	{
		$$ = []ExprSwitchStmt{"switch", $2, ";", $4, "{", $6, "}"} //TODO 100
	}

ExprSwitchStmt2:
	/* EMPTY */
	{
		$$ = nil //TODO 101
	}
|	Expression
	{
		$$ = $1 //TODO 102
	}

ExprSwitchStmt3:
	/* EMPTY */
	{
		$$ = []ExprSwitchStmt3(nil) //TODO 103
	}
|	ExprSwitchStmt3 ExprCaseClause
	{
		$$ = append($1.([]ExprSwitchStmt3), $2) //TODO 104
	}

Expression:
	UnaryExpr
	{
		$$ = $1 //TODO 105
	}
|	Expression Binary_op UnaryExpr
	{
		$$ = []Expression{$1, $2, $3} //TODO 106
	}

ExpressionList:
	Expression ExpressionList1
	{
		$$ = []ExpressionList{$1, $2} //TODO 107
	}

ExpressionList1:
	/* EMPTY */
	{
		$$ = []ExpressionList1(nil) //TODO 108
	}
|	ExpressionList1 ',' Expression
	{
		$$ = append($1.([]ExpressionList1), ",", $3) //TODO 109
	}

ExpressionStmt:
	Expression
	{
		$$ = $1 //TODO 110
	}

FallthroughStmt:
	FALLTHROUGH
	{
		$$ = "fallthrough" //TODO 111
	}

FieldDecl:
	FieldDecl1 FieldDecl2
	{
		$$ = []FieldDecl{$1, $2} //TODO 112
	}

FieldDecl1:
	IdentifierList Type
	{
		$$ = []FieldDecl1{$1, $2} //TODO 113
	}
|	AnonymousField
	{
		$$ = $1 //TODO 114
	}

FieldDecl2:
	/* EMPTY */
	{
		$$ = nil //TODO 115
	}
|	Tag
	{
		$$ = $1 //TODO 116
	}

FieldName:
	Dclname
	{
		$$ = $1 //TODO 117
	}

ForClause:
	ForClause1 ';' ForClause2 ';' ForClause3
	{
		$$ = []ForClause{$1, ";", $3, ";", $5} //TODO 118
	}

ForClause1:
	/* EMPTY */
	{
		$$ = nil //TODO 119
	}
|	InitStmt
	{
		$$ = $1 //TODO 120
	}

ForClause2:
	/* EMPTY */
	{
		$$ = nil //TODO 121
	}
|	Condition
	{
		$$ = $1 //TODO 122
	}

ForClause3:
	/* EMPTY */
	{
		$$ = nil //TODO 123
	}
|	PostStmt
	{
		$$ = $1 //TODO 124
	}

ForStmt:
	FOR ForStmt1 Block
	{
		$$ = []ForStmt{"for", $2, $3} //TODO 125
	}

ForStmt1:
	/* EMPTY */
	{
		$$ = nil //TODO 126
	}
|	ForStmt11
	{
		$$ = $1 //TODO 127
	}

ForStmt11:
	Condition
	{
		$$ = $1 //TODO 128
	}
|	ForClause
	{
		$$ = $1 //TODO 129
	}
|	RangeClause
	{
		$$ = $1 //TODO 130
	}

Function:
	Signature FunctionBody
	{
		$$ = []Function{$1, $2} //TODO 131
	}

FunctionBody:
	Block
	{
		$$ = $1 //TODO 132
	}

FunctionDecl:
	FUNC FunctionName FunctionDecl1
	{
		$$ = []FunctionDecl{"func", $2, $3} //TODO 133
	}

FunctionDecl1:
	Function
	{
		$$ = $1 //TODO 134
	}
|	Signature
	{
		$$ = $1 //TODO 135
	}

FunctionLit:
	FUNC Function
	{
		$$ = []FunctionLit{"func", $2} //TODO 136
	}

FunctionName:
	Dclname
	{
		$$ = $1 //TODO 137
	}

FunctionType:
	FUNC Signature
	{
		$$ = []FunctionType{"func", $2} //TODO 138
	}

GoStmt:
	GO Expression
	{
		$$ = []GoStmt{"go", $2} //TODO 139
	}

GotoStmt:
	GOTO Label
	{
		$$ = []GotoStmt{"goto", $2} //TODO 140
	}

IdentifierList:
	Dclname IdentifierList1
	{
		$$ = []IdentifierList{$1, $2} //TODO 141
	}

IdentifierList1:
	/* EMPTY */
	{
		$$ = []IdentifierList1(nil) //TODO 142
	}
|	IdentifierList1 ',' Dclname
	{
		$$ = append($1.([]IdentifierList1), ",", $3) //TODO 143
	}

IfStmt:
	IF Expression Block IfStmt2
	{
		$$ = []IfStmt{"if", $2, $3, $4} //TODO 144
	}
|	IF SimpleStmt ';' Expression Block IfStmt2
	{
		$$ = []IfStmt{"if", $2, ";", $4, $5, $6} //TODO 145
	}

IfStmt2:
	/* EMPTY */
	{
		$$ = nil //TODO 146
	}
|	ELSE IfStmt21
	{
		$$ = []IfStmt2{"else", $2} //TODO 147
	}

IfStmt21:
	IfStmt
	{
		$$ = $1 //TODO 148
	}
|	Block
	{
		$$ = $1 //TODO 149
	}

ImportDecl:
	IMPORT ImportDecl1
	{
		$$ = []ImportDecl{"import", $2} //TODO 150
	}

ImportDecl1:
	ImportSpec
	{
		$$ = $1 //TODO 151
	}
|	'(' ImportDecl11 ')'
	{
		$$ = []ImportDecl1{"(", $2, ")"} //TODO 152
	}

ImportDecl11:
	/* EMPTY */
	{
		$$ = []ImportDecl11(nil) //TODO 153
	}
|	ImportDecl11 ImportSpec ';'
	{
		$$ = append($1.([]ImportDecl11), $2, ";") //TODO 154
	}

ImportPath:
	STRING_LIT
	{
		$$ = $1 //TODO 155
	}

ImportSpec:
	ImportSpec1 ImportPath
	{
		$$ = []ImportSpec{$1, $2} //TODO 156
	}

ImportSpec1:
	/* EMPTY */
	{
		$$ = nil //TODO 157
	}
|	ImportSpec11
	{
		$$ = $1 //TODO 158
	}

ImportSpec11:
	'.'
	{
		$$ = "." //TODO 159
	}
|	Dclname
	{
		$$ = $1 //TODO 160
	}

IncDecStmt:
	Expression IncDecStmt1
	{
		$$ = []IncDecStmt{$1, $2} //TODO 161
	}

IncDecStmt1:
	INC
	{
		$$ = $1 //TODO 162
	}
|	DEC
	{
		$$ = $1 //TODO 163
	}

Index:
	'[' Expression ']'
	{
		$$ = []Index{"[", $2, "]"} //TODO 164
	}

InitStmt:
	SimpleStmt
	{
		$$ = $1 //TODO 165
	}

InterfaceType:
	INTERFACE '{' InterfaceType1 '}'
	{
		$$ = []InterfaceType{"interface", "{", $3, "}"} //TODO 166
	}

InterfaceType1:
	/* EMPTY */
	{
		$$ = []InterfaceType1(nil) //TODO 167
	}
|	InterfaceType1 MethodSpec ';'
	{
		$$ = append($1.([]InterfaceType1), $2, ";") //TODO 168
	}

InterfaceTypeName:
	TYPE_NAME
	{
		$$ = $1 //TODO 169
	}
|	PACK_NAME '.' TYPE_NAME
	{
		$$ = []InterfaceTypeName{$1, ".", $3} //TODO 170
	}

Key:
	FieldName
	{
		$$ = $1 //TODO 171
	}
|	ElementIndex
	{
		$$ = $1 //TODO 172
	}

KeyType:
	Type
	{
		$$ = $1 //TODO 173
	}

Label:
	Dclname
	{
		$$ = $1 //TODO 174
	}

Literal:
	BasicLit
	{
		$$ = $1 //TODO 175
	}
|	CompositeLit
	{
		$$ = $1 //TODO 176
	}
|	FunctionLit
	{
		$$ = $1 //TODO 177
	}

LiteralType:
	StructType
	{
		$$ = $1 //TODO 178
	}
|	ArrayType
	{
		$$ = $1 //TODO 179
	}
|	'[' DDD ']' ElementType
	{
		$$ = []LiteralType{"[", $2, "]", $4} //TODO 180
	}
|	SliceType
	{
		$$ = $1 //TODO 181
	}
|	MapType
	{
		$$ = $1 //TODO 182
	}
|	TYPE_NAME
	{
		$$ = $1 //TODO 183
	}
|	PACK_NAME '.' TYPE_NAME
	{
		$$ = []LiteralType{$1, ".", $3} //TODO 184
	}

LiteralValue:
	'{' LiteralValue1 '}'
	{
		$$ = []LiteralValue{"{", $2, "}"} //TODO 185
	}

LiteralValue1:
	/* EMPTY */
	{
		$$ = nil //TODO 186
	}
|	Element ElementList1 LiteralValue11
	{
		$$ = []LiteralValue1{$1, $2, $3} //TODO 187
	}

LiteralValue11:
	/* EMPTY */
	{
		$$ = nil //TODO 188
	}
|	','
	{
		$$ = "," //TODO 189
	}

MapType:
	MAP '[' KeyType ']' ElementType
	{
		$$ = []MapType{"map", "[", $3, "]", $5} //TODO 190
	}

MethodDecl:
	FUNC Receiver MethodName MethodDecl1
	{
		$$ = []MethodDecl{"func", $2, $3, $4} //TODO 191
	}

MethodDecl1:
	Function
	{
		$$ = $1 //TODO 192
	}
|	Signature
	{
		$$ = $1 //TODO 193
	}

MethodExpr:
	ReceiverType '.' MethodName
	{
		$$ = []MethodExpr{$1, ".", $3} //TODO 194
	}

MethodName:
	Dclname
	{
		$$ = $1 //TODO 195
	}

MethodSpec:
	MethodName Signature
	{
		$$ = []MethodSpec{$1, $2} //TODO 196
	}
|	InterfaceTypeName
	{
		$$ = $1 //TODO 197
	}

Operand:
	Literal
	{
		$$ = $1 //TODO 198
	}
|	OperandName
	{
		$$ = $1 //TODO 199
	}
|	MethodExpr
	{
		$$ = $1 //TODO 200
	}
|	'(' Expression ')'
	{
		$$ = []Operand{"(", $2, ")"} //TODO 201
	}

OperandName:
	NAME
	{
		$$ = $1 //TODO 202
	}
|	PACK_NAME '.' NAME
	{
		$$ = []OperandName{$1, ".", $3} //TODO 203
	}

PackageClause:
	PACKAGE Dclname
	{
		$$ = []PackageClause{"package", $2} //TODO 204
	}

ParameterDecl:
	ParameterDecl1 ParameterDecl2 Type
	{
		$$ = []ParameterDecl{$1, $2, $3} //TODO 205
	}

ParameterDecl1:
	/* EMPTY */
	{
		$$ = nil //TODO 206
	}
|	IdentifierList
	{
		$$ = $1 //TODO 207
	}

ParameterDecl2:
	/* EMPTY */
	{
		$$ = nil //TODO 208
	}
|	DDD
	{
		$$ = $1 //TODO 209
	}

ParameterList1:
	/* EMPTY */
	{
		$$ = []ParameterList1(nil) //TODO 210
	}
|	ParameterList1 ',' ParameterDecl
	{
		$$ = append($1.([]ParameterList1), ",", $3) //TODO 211
	}

Parameters:
	'(' Parameters1 ')'
	{
		$$ = []Parameters{"(", $2, ")"} //TODO 212
	}

Parameters1:
	/* EMPTY */
	{
		$$ = nil //TODO 213
	}
|	ParameterDecl ParameterList1 Parameters11
	{
		$$ = []Parameters1{$1, $2, $3} //TODO 214
	}

Parameters11:
	/* EMPTY */
	{
		$$ = nil //TODO 215
	}
|	','
	{
		$$ = "," //TODO 216
	}

PointerType:
	'*' BaseType
	{
		$$ = []PointerType{"*", $2} //TODO 217
	}

PostStmt:
	SimpleStmt
	{
		$$ = $1 //TODO 218
	}

PrimaryExpr:
	Operand
	{
		$$ = $1 //TODO 219
	}
|	Conversion
	{
		$$ = $1 //TODO 220
	}
|	BuiltinCall
	{
		$$ = $1 //TODO 221
	}
|	PrimaryExpr Selector
	{
		$$ = []PrimaryExpr{$1, $2} //TODO 222
	}
|	PrimaryExpr Index
	{
		$$ = []PrimaryExpr{$1, $2} //TODO 223
	}
|	PrimaryExpr Slice
	{
		$$ = []PrimaryExpr{$1, $2} //TODO 224
	}
|	PrimaryExpr TypeAssertion
	{
		$$ = []PrimaryExpr{$1, $2} //TODO 225
	}
|	PrimaryExpr Call
	{
		$$ = []PrimaryExpr{$1, $2} //TODO 226
	}

RangeClause:
	RangeClause1 RANGE Expression
	{
		$$ = []RangeClause{$1, "range", $3} //TODO 227
	}

RangeClause1:
	ExpressionList '='
	{
		$$ = []RangeClause1{$1, "="} //TODO 228
	}
|	IdentifierList COLAS
	{
		$$ = []RangeClause1{$1, $2} //TODO 229
	}

Receiver:
	'(' '*' Dclname ')'
	{
		$$ = []Receiver{"(", "*", $3, ")"} //TODO 230
	}
|	'(' Dclname ')'
	{
		$$ = []Receiver{"(", $2, ")"} //TODO 231
	}
|	'(' Dclname '*' Dclname ')'
	{
		$$ = []Receiver{"(", $2, "*", $4, ")"} //TODO 232
	}
|	'(' Dclname Dclname ')'
	{
		$$ = []Receiver{"(", $2, $3, ")"} //TODO 233
	}

ReceiverType:
	TYPE_NAME
	{
		$$ = $1 //TODO 234
	}
|	PACK_NAME '.' TYPE_NAME
	{
		$$ = []ReceiverType{$1, ".", $3} //TODO 235
	}
|	'(' '*' TYPE_NAME ')'
	{
		$$ = []ReceiverType{"(", "*", $3, ")"} //TODO 236
	}
|	'(' '*' PACK_NAME '.' TYPE_NAME ')'
	{
		$$ = []ReceiverType{"(", "*", $3, ".", $5, ")"} //TODO 237
	}
|	'(' TYPE_NAME ')'
	{
		$$ = []ReceiverType{"(", $2, ")"} //TODO 238
	}
|	'(' PACK_NAME '.' TYPE_NAME ')'
	{
		$$ = []ReceiverType{"(", $2, ".", $4, ")"} //TODO 239
	}

RecvExpr:
	Expression
	{
		$$ = $1 //TODO 240
	}

RecvStmt:
	RecvExpr
	{
		$$ = $1 //TODO 241
	}
|	RecvStmt11 RecvExpr
	{
		$$ = []RecvStmt{$1, $2} //TODO 242
	}

RecvStmt11:
	ExpressionList '='
	{
		$$ = []RecvStmt11{$1, "="} //TODO 243
	}
|	IdentifierList COLAS
	{
		$$ = []RecvStmt11{$1, $2} //TODO 244
	}

ReturnStmt:
	RETURN ReturnStmt1
	{
		$$ = []ReturnStmt{"return", $2} //TODO 245
	}

ReturnStmt1:
	/* EMPTY */
	{
		$$ = nil //TODO 246
	}
|	ExpressionList
	{
		$$ = $1 //TODO 247
	}

SelectStmt:
	SELECT '{' SelectStmt1 '}'
	{
		$$ = []SelectStmt{"select", "{", $3, "}"} //TODO 248
	}

SelectStmt1:
	/* EMPTY */
	{
		$$ = []SelectStmt1(nil) //TODO 249
	}
|	SelectStmt1 CommClause
	{
		$$ = append($1.([]SelectStmt1), $2) //TODO 250
	}

Selector:
	'.' Dclname
	{
		$$ = []Selector{".", $2} //TODO 251
	}

SendStmt:
	Channel COMM Expression
	{
		$$ = []SendStmt{$1, $2, $3} //TODO 252
	}

ShortVarDecl:
	IdentifierList COLAS ExpressionList
	{
		$$ = []ShortVarDecl{$1, $2, $3} //TODO 253
	}

Signature:
	Parameters Signature1
	{
		$$ = []Signature{$1, $2} //TODO 254
	}

Signature1:
	/* EMPTY */
	{
		$$ = nil //TODO 255
	}
|	Signature11
	{
		$$ = $1 //TODO 256
	}

Signature11:
	Parameters
	{
		$$ = $1 //TODO 257
	}
|	TYPE_NAME
	{
		$$ = $1 //TODO 258
	}
|	PACK_NAME '.' TYPE_NAME
	{
		$$ = []Signature11{$1, ".", $3} //TODO 259
	}
|	TypeLit
	{
		$$ = $1 //TODO 260
	}

SimpleStmt:
	ExpressionStmt
	{
		$$ = $1 //TODO 261
	}
|	SendStmt
	{
		$$ = $1 //TODO 262
	}
|	IncDecStmt
	{
		$$ = $1 //TODO 263
	}
|	Assignment
	{
		$$ = $1 //TODO 264
	}
|	ShortVarDecl
	{
		$$ = $1 //TODO 265
	}

Slice:
	'[' Slice1 ':' Slice2 ']'
	{
		$$ = []Slice{"[", $2, ":", $4, "]"} //TODO 266
	}

Slice1:
	/* EMPTY */
	{
		$$ = nil //TODO 267
	}
|	Expression
	{
		$$ = $1 //TODO 268
	}

Slice2:
	/* EMPTY */
	{
		$$ = nil //TODO 269
	}
|	Expression
	{
		$$ = $1 //TODO 270
	}

SliceType:
	'[' ']' ElementType
	{
		$$ = []SliceType{"[", "]", $3} //TODO 271
	}

SourceFile:
	PackageClause ';' SourceFile1 SourceFile2
	{
		$$ = []SourceFile{$1, ";", $3, $4} //TODO 272
	}

SourceFile1:
	/* EMPTY */
	{
		$$ = []SourceFile1(nil) //TODO 273
	}
|	SourceFile1 ImportDecl ';'
	{
		$$ = append($1.([]SourceFile1), $2, ";") //TODO 274
	}

SourceFile2:
	/* EMPTY */
	{
		$$ = []SourceFile2(nil) //TODO 275
	}
|	SourceFile2 TopLevelDecl ';'
	{
		$$ = append($1.([]SourceFile2), $2, ";") //TODO 276
	}

Start:
	SourceFile
	{
		_parserResult = $1 //TODO 277
	}

Statement:
	Declaration
	{
		$$ = $1 //TODO 278
	}
|	Label ':'
	{
		$$ = []Statement{$1, ":"} //TODO 279
	}
|	SimpleStmt
	{
		$$ = $1 //TODO 280
	}
|	GoStmt
	{
		$$ = $1 //TODO 281
	}
|	ReturnStmt
	{
		$$ = $1 //TODO 282
	}
|	BreakStmt
	{
		$$ = $1 //TODO 283
	}
|	ContinueStmt
	{
		$$ = $1 //TODO 284
	}
|	GotoStmt
	{
		$$ = $1 //TODO 285
	}
|	FallthroughStmt
	{
		$$ = $1 //TODO 286
	}
|	Block
	{
		$$ = $1 //TODO 287
	}
|	IfStmt
	{
		$$ = $1 //TODO 288
	}
|	SwitchStmt
	{
		$$ = $1 //TODO 289
	}
|	SelectStmt
	{
		$$ = $1 //TODO 290
	}
|	ForStmt
	{
		$$ = $1 //TODO 291
	}
|	DeferStmt
	{
		$$ = $1 //TODO 292
	}

StatementList:
	StatementList1
	{
		$$ = $1 //TODO 293
	}

StatementList1:
	/* EMPTY */
	{
		$$ = []StatementList1(nil) //TODO 294
	}
|	StatementList1 StatementList11 ';'
	{
		$$ = append($1.([]StatementList1), $2, ";") //TODO 295
	}

StatementList11:
	/* EMPTY */
	{
		$$ = nil //TODO 296
	}
|	Statement
	{
		$$ = $1 //TODO 297
	}

StructType:
	STRUCT '{' StructType1 '}'
	{
		$$ = []StructType{"struct", "{", $3, "}"} //TODO 298
	}

StructType1:
	/* EMPTY */
	{
		$$ = []StructType1(nil) //TODO 299
	}
|	StructType1 FieldDecl ';'
	{
		$$ = append($1.([]StructType1), $2, ";") //TODO 300
	}

SwitchStmt:
	ExprSwitchStmt
	{
		$$ = $1 //TODO 301
	}
|	TypeSwitchStmt
	{
		$$ = $1 //TODO 302
	}

Tag:
	STRING_LIT
	{
		$$ = $1 //TODO 303
	}

TopLevelDecl:
	Declaration
	{
		$$ = $1 //TODO 304
	}
|	FunctionDecl
	{
		$$ = $1 //TODO 305
	}
|	MethodDecl
	{
		$$ = $1 //TODO 306
	}

Type:
	TYPE_NAME
	{
		$$ = $1 //TODO 307
	}
|	PACK_NAME '.' TYPE_NAME
	{
		$$ = []Type{$1, ".", $3} //TODO 308
	}
|	TypeLit
	{
		$$ = $1 //TODO 309
	}
|	'(' Type ')'
	{
		$$ = []Type{"(", $2, ")"} //TODO 310
	}

TypeAssertion:
	'.' '(' Type ')'
	{
		$$ = []TypeAssertion{".", "(", $3, ")"} //TODO 311
	}

TypeCaseClause:
	TypeSwitchCase ':' StatementList
	{
		$$ = []TypeCaseClause{$1, ":", $3} //TODO 312
	}

TypeDecl:
	TYPE TypeDecl1
	{
		$$ = []TypeDecl{"type", $2} //TODO 313
	}

TypeDecl1:
	TypeSpec
	{
		$$ = $1 //TODO 314
	}
|	'(' TypeDecl11 ')'
	{
		$$ = []TypeDecl1{"(", $2, ")"} //TODO 315
	}

TypeDecl11:
	/* EMPTY */
	{
		$$ = []TypeDecl11(nil) //TODO 316
	}
|	TypeDecl11 TypeSpec ';'
	{
		$$ = append($1.([]TypeDecl11), $2, ";") //TODO 317
	}

TypeList:
	Type TypeList1
	{
		$$ = []TypeList{$1, $2} //TODO 318
	}

TypeList1:
	/* EMPTY */
	{
		$$ = []TypeList1(nil) //TODO 319
	}
|	TypeList1 ',' Type
	{
		$$ = append($1.([]TypeList1), ",", $3) //TODO 320
	}

TypeLit:
	ArrayType
	{
		$$ = $1 //TODO 321
	}
|	StructType
	{
		$$ = $1 //TODO 322
	}
|	PointerType
	{
		$$ = $1 //TODO 323
	}
|	FunctionType
	{
		$$ = $1 //TODO 324
	}
|	InterfaceType
	{
		$$ = $1 //TODO 325
	}
|	SliceType
	{
		$$ = $1 //TODO 326
	}
|	MapType
	{
		$$ = $1 //TODO 327
	}
|	ChannelType
	{
		$$ = $1 //TODO 328
	}

TypeSpec:
	Dclname Type
	{
		$$ = []TypeSpec{$1, $2} //TODO 329
	}

TypeSwitchCase:
	CASE TypeList
	{
		$$ = []TypeSwitchCase{"case", $2} //TODO 330
	}
|	DEFAULT
	{
		$$ = "default" //TODO 331
	}

TypeSwitchGuard:
	PrimaryExpr '.' '(' TYPE ')'
	{
		$$ = []TypeSwitchGuard{$1, ".", "(", "type", ")"} //TODO 332
	}
|	Dclname COLAS PrimaryExpr '.' '(' TYPE ')'
	{
		$$ = []TypeSwitchGuard{$1, $2, $3, ".", "(", "type", ")"} //TODO 333
	}

TypeSwitchStmt:
	SWITCH TypeSwitchGuard '{' TypeSwitchStmt2 '}'
	{
		$$ = []TypeSwitchStmt{"switch", $2, "{", $4, "}"} //TODO 334
	}
|	SWITCH SimpleStmt ';' TypeSwitchGuard '{' TypeSwitchStmt2 '}'
	{
		$$ = []TypeSwitchStmt{"switch", $2, ";", $4, "{", $6, "}"} //TODO 335
	}

TypeSwitchStmt2:
	/* EMPTY */
	{
		$$ = []TypeSwitchStmt2(nil) //TODO 336
	}
|	TypeSwitchStmt2 TypeCaseClause
	{
		$$ = append($1.([]TypeSwitchStmt2), $2) //TODO 337
	}

UnaryExpr:
	PrimaryExpr
	{
		$$ = $1 //TODO 338
	}
|	'+' UnaryExpr
	{
		$$ = []UnaryExpr{"+", $2} //TODO 339
	}
|	'-' UnaryExpr
	{
		$$ = []UnaryExpr{"-", $2} //TODO 340
	}
|	'!' UnaryExpr
	{
		$$ = []UnaryExpr{"!", $2} //TODO 341
	}
|	'^' UnaryExpr
	{
		$$ = []UnaryExpr{"^", $2} //TODO 342
	}
|	'*' UnaryExpr
	{
		$$ = []UnaryExpr{"*", $2} //TODO 343
	}
|	'&' UnaryExpr
	{
		$$ = []UnaryExpr{"&", $2} //TODO 344
	}
|	COMM UnaryExpr
	{
		$$ = []UnaryExpr{$1, $2} //TODO 345
	}

Value:
	Expression
	{
		$$ = $1 //TODO 346
	}
|	LiteralValue
	{
		$$ = $1 //TODO 347
	}

VarDecl:
	VAR VarDecl1
	{
		$$ = []VarDecl{"var", $2} //TODO 348
	}

VarDecl1:
	VarSpec
	{
		$$ = $1 //TODO 349
	}
|	'(' VarDecl11 ')'
	{
		$$ = []VarDecl1{"(", $2, ")"} //TODO 350
	}

VarDecl11:
	/* EMPTY */
	{
		$$ = []VarDecl11(nil) //TODO 351
	}
|	VarDecl11 VarSpec ';'
	{
		$$ = append($1.([]VarDecl11), $2, ";") //TODO 352
	}

VarSpec:
	IdentifierList VarSpec1
	{
		$$ = []VarSpec{$1, $2} //TODO 353
	}

VarSpec1:
	Type VarSpec11
	{
		$$ = []VarSpec1{$1, $2} //TODO 354
	}
|	'=' ExpressionList
	{
		$$ = []VarSpec1{"=", $2} //TODO 355
	}

VarSpec11:
	/* EMPTY */
	{
		$$ = nil //TODO 356
	}
|	'=' ExpressionList
	{
		$$ = []VarSpec11{"=", $2} //TODO 357
	}

%%

//TODO remove demo stuff below

var _parserResult interface{}

type (
	AnonymousField interface{}
	ArgumentList interface{}
	ArgumentList1 interface{}
	ArrayLength interface{}
	ArrayType interface{}
	Assignment interface{}
	BaseType interface{}
	BasicLit interface{}
	Binary_op interface{}
	Block interface{}
	BreakStmt interface{}
	BreakStmt1 interface{}
	BuiltinArgs interface{}
	BuiltinArgs1 interface{}
	BuiltinCall interface{}
	BuiltinCall1 interface{}
	BuiltinCall11 interface{}
	Call interface{}
	Call1 interface{}
	Call11 interface{}
	Channel interface{}
	ChannelType interface{}
	ChannelType1 interface{}
	CommCase interface{}
	CommCase1 interface{}
	CommClause interface{}
	CompositeLit interface{}
	Condition interface{}
	ConstDecl interface{}
	ConstDecl1 interface{}
	ConstDecl11 interface{}
	ConstSpec interface{}
	ConstSpec1 interface{}
	ConstSpec11 interface{}
	ContinueStmt interface{}
	ContinueStmt1 interface{}
	Conversion interface{}
	Conversion1 interface{}
	Dclname interface{}
	Declaration interface{}
	DeferStmt interface{}
	Element interface{}
	ElementIndex interface{}
	ElementList1 interface{}
	ElementType interface{}
	ExprCaseClause interface{}
	ExprSwitchCase interface{}
	ExprSwitchStmt interface{}
	ExprSwitchStmt2 interface{}
	ExprSwitchStmt3 interface{}
	Expression interface{}
	ExpressionList interface{}
	ExpressionList1 interface{}
	ExpressionStmt interface{}
	FallthroughStmt interface{}
	FieldDecl interface{}
	FieldDecl1 interface{}
	FieldDecl2 interface{}
	FieldName interface{}
	ForClause interface{}
	ForClause1 interface{}
	ForClause2 interface{}
	ForClause3 interface{}
	ForStmt interface{}
	ForStmt1 interface{}
	ForStmt11 interface{}
	Function interface{}
	FunctionBody interface{}
	FunctionDecl interface{}
	FunctionDecl1 interface{}
	FunctionLit interface{}
	FunctionName interface{}
	FunctionType interface{}
	GoStmt interface{}
	GotoStmt interface{}
	IdentifierList interface{}
	IdentifierList1 interface{}
	IfStmt interface{}
	IfStmt2 interface{}
	IfStmt21 interface{}
	ImportDecl interface{}
	ImportDecl1 interface{}
	ImportDecl11 interface{}
	ImportPath interface{}
	ImportSpec interface{}
	ImportSpec1 interface{}
	ImportSpec11 interface{}
	IncDecStmt interface{}
	IncDecStmt1 interface{}
	Index interface{}
	InitStmt interface{}
	InterfaceType interface{}
	InterfaceType1 interface{}
	InterfaceTypeName interface{}
	Key interface{}
	KeyType interface{}
	Label interface{}
	Literal interface{}
	LiteralType interface{}
	LiteralValue interface{}
	LiteralValue1 interface{}
	LiteralValue11 interface{}
	MapType interface{}
	MethodDecl interface{}
	MethodDecl1 interface{}
	MethodExpr interface{}
	MethodName interface{}
	MethodSpec interface{}
	Operand interface{}
	OperandName interface{}
	PackageClause interface{}
	ParameterDecl interface{}
	ParameterDecl1 interface{}
	ParameterDecl2 interface{}
	ParameterList1 interface{}
	Parameters interface{}
	Parameters1 interface{}
	Parameters11 interface{}
	PointerType interface{}
	PostStmt interface{}
	PrimaryExpr interface{}
	RangeClause interface{}
	RangeClause1 interface{}
	Receiver interface{}
	ReceiverType interface{}
	RecvExpr interface{}
	RecvStmt interface{}
	RecvStmt11 interface{}
	ReturnStmt interface{}
	ReturnStmt1 interface{}
	SelectStmt interface{}
	SelectStmt1 interface{}
	Selector interface{}
	SendStmt interface{}
	ShortVarDecl interface{}
	Signature interface{}
	Signature1 interface{}
	Signature11 interface{}
	SimpleStmt interface{}
	Slice interface{}
	Slice1 interface{}
	Slice2 interface{}
	SliceType interface{}
	SourceFile interface{}
	SourceFile1 interface{}
	SourceFile2 interface{}
	Start interface{}
	Statement interface{}
	StatementList interface{}
	StatementList1 interface{}
	StatementList11 interface{}
	StructType interface{}
	StructType1 interface{}
	SwitchStmt interface{}
	Tag interface{}
	TopLevelDecl interface{}
	Type interface{}
	TypeAssertion interface{}
	TypeCaseClause interface{}
	TypeDecl interface{}
	TypeDecl1 interface{}
	TypeDecl11 interface{}
	TypeList interface{}
	TypeList1 interface{}
	TypeLit interface{}
	TypeSpec interface{}
	TypeSwitchCase interface{}
	TypeSwitchGuard interface{}
	TypeSwitchStmt interface{}
	TypeSwitchStmt2 interface{}
	UnaryExpr interface{}
	Value interface{}
	VarDecl interface{}
	VarDecl1 interface{}
	VarDecl11 interface{}
	VarSpec interface{}
	VarSpec1 interface{}
	VarSpec11 interface{}
)
	
func _dump() {
	s := fmt.Sprintf("%#v", _parserResult)
	s = strings.Replace(s, "%", "%%", -1)
	s = strings.Replace(s, "{", "{%i\n", -1)
	s = strings.Replace(s, "}", "%u\n}", -1)
	s = strings.Replace(s, ", ", ",\n", -1)
	var buf bytes.Buffer
	strutil.IndentFormatter(&buf, ". ").Format(s)
	buf.WriteString("\n")
	a := strings.Split(buf.String(), "\n")
	for _, v := range a {
		if strings.HasSuffix(v, "(nil)") || strings.HasSuffix(v, "(nil),") {
			continue
		}
	
		fmt.Println(v)
	}
}

// End of demo stuff
